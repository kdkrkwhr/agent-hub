...
async function api(path,body){const r=await fetch(path,body===undefined?{}:{method:'POST',headers:{'Content-Type':'application/json','X-Hub-CSRF':csrf},body:JSON.stringify(body)});const data=await r.json();if(!r.ok)throw Error(data.error||'요청 실패');return data;}
...
function render(){
 ...
 const key=JSON.stringify([snapshot.threads,snapshot.jobs,snapshot.active,selected,agent,tab,$('search').value]);if(key===lastKey)return;lastKey=key;
 ...
 [...threads].reverse().forEach(t=>channel(t.threadId,t.threadName,`${t.messages?.length||0}개 메시지 · ${(t.participatingAgents||[]).join(' / ')}`));$('channel-count').textContent=threads.length;
 ...
 $('send').disabled=!selected||busy;$('message').disabled=!selected;
 ...
 for(const job of snapshot.jobs){const card=node('article','job'),head=node('div','job-head');head.append(node('strong','',job.agent.toUpperCase()),node('span','badge',job.status),node('small','',job.id));card.append(head);if(job.error)card.append(node('p','error',job.error));
  ...
  if(['running','pending'].includes(job.status))action('취소','/api/cancel');if(['failed','cancelled'].includes(job.status))action('재시도','/api/retry');
