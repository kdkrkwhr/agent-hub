...
function renderPipelineCard(p,result){
 ...
 if(p.artifacts&&p.status==='completed'){const publish=node('button','poll-room-link','원본 반영 · 커밋 · push');publish.dataset.publish=p.id;publish.onclick=()=>openPublication(p.id);card.append(publish);if(p.publication)card.append(node('p','hint','원본 반영 상태: '+({ready:'미리보기 확인',applying:'반영 상태 확인 필요',applied:'반영 · 스테이징 완료',committing:'커밋 상태 확인 필요',committed:'커밋 완료',pushed:'push 완료'}[p.publication.stage]||p.publication.stage)+(p.publication.commit?' · '+p.publication.commit.slice(0,10):'')));}
...
async function openPublication(id){
 publicationPreview=null;$('publish-error').textContent='';$('publish-target').textContent='원본과 변경 내용을 확인하는 중…';$('publish-state').textContent='';$('publish-files').replaceChildren();$('publish-diff').textContent='';$('publish-truncated').textContent='';$('publish-run').disabled=true;$('publish-confirm').checked=false;$('publish-action').value='apply';$('publish-message').value='';$('publish-message-label').hidden=true;$('publish-dialog').showModal();
 try{const p=await api('/api/pipeline/publish/preview',{id});publicationPreview=p;$('publish-target').textContent='저장소: '+p.source+' · 브랜치: '+p.branch+' · origin: '+(p.remote||'없음');$('publish-state').textContent='현재 단계: '+({ready:'미반영',applied:'반영 · 스테이징 완료',committed:'커밋 완료',pushed:'push 완료'}[p.stage]||p.stage)+' · '+(p.test_status==='skipped'?'테스트 미실행':'검증 기록 있음');for(const name of p.files)$('publish-files').append(node('div','',name));$('publish-diff').textContent=p.diff;$('publish-truncated').textContent=p.diff_truncated?'긴 diff는 앞부분만 표시합니다. 전체 변경 패치를 다운로드해서 확인할 수 있습니다.':'';for(const o of $('publish-action').options)o.disabled=(o.value==='push'&&!p.remote)||(p.stage==='committed'&&o.value!=='push')||p.stage==='pushed';$('publish-action').value=p.stage==='committed'?'push':p.stage==='applied'?'commit':'apply';updatePublicationAction();$('publish-run').disabled=p.stage==='pushed'||(p.stage==='committed'&&!p.remote);}catch(e){$('publish-error').textContent=e.message;}
}
function updatePublicationAction(){const need=$('publish-action').value!=='apply'&&publicationPreview?.stage!=='committed';$('publish-message-label').hidden=!need;$('publish-message').required=need;$('publish-confirm').checked=false;}
